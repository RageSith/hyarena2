...
public class EconomyManager {
     ...
     */
    public MatchRewardResult rewardMatch(UUID uuid, boolean isWinner, int kills, String matchId) {
        PlayerEconomyData data = playerDataManager.getData(uuid);
        ...
        // Apply AP
        addArenaPoints(uuid, totalAp, "Match " + matchId + (isWinner ? " (win)" : " (loss)"));
    ...
    private void onMatchEnded(MatchEndedEvent event) {
        ...
        for (Participant participant : match.getParticipants()) {
            ...
            MatchRewardResult result = rewardMatch(playerUuid, isWinner, kills, event.getMatchId().toString().substring(0, 8));

            // Send reward message to player
            participant.sendMessage(
                "<color:#f1c40f>+" + result.getApEarned() + " AP</color>"
                + " <color:#7f8c8d>|</color> "
