...
public class ShopItemEditorPage extends InteractiveCustomUIPage<ShopItemEditorPage.PageEventData> implements CloseablePage {
    ...
    public ShopItemEditorPage(PlayerRef playerRef, UUID playerUuid,
        ...
        if (existingItem != null) {
            ...
            if (catId != null) {
                ShopCategory cat = shopManager.getCategory(catId);
                formCategoryName = cat != null ? cat.getDisplayName() : "";
            } else {
                formCategoryName = "";
            }
        } else {
    ...
    public void build(@Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder cmd,
        ...
        // Category
        cmd.set("#CategoryIdField.Value", formCategoryId);
        cmd.set("#CategoryNameField.Value", formCategoryName);
        ...
        bindTextField(events, "#CategoryIdField", "categoryId");
        bindTextField(events, "#CategoryNameField", "categoryName");
    ...
    private void handleFieldChange(PageEventData data) {
        ...
        switch (data.field) {
            ...
            case "categoryId":
                ...
                break;
            case "categoryName":
                if (data.value != null) formCategoryName = data.value;
                break;
            case "cost":
    ...
    private void handleSave() {
        ...
        if (formCategoryId == null || formCategoryId.trim().isEmpty()) {
            showStatus("Category ID is required", "#e74c3c");
            return;
        }
        ...
        shopManager.saveItem(item, formCategoryId.trim(), formCategoryName.trim());
