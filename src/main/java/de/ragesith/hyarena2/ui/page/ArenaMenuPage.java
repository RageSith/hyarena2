...
public class ArenaMenuPage extends InteractiveCustomUIPage<ArenaMenuPage.PageEventData> implements CloseablePage {
    ...
    public void build(@Nonnull Ref<EntityStore> ref, @Nonnull UICommandBuilder cmd,
        ...
        for (int i = 0; i < filteredArenas.size(); i++) {
            ...
            GameMode gm = matchManager.getGameMode(arena.getGameMode());
            String gmName = gm != null ? gm.getDisplayName() : arena.getGameMode();
            cmd.set(row + " #PlayerCount.Text", gmName + "  |  " + MatchManager.formatPlayerCount(arena));

            // Bots tag
            if (arena.getConfig().isAutoFillEnabled()) {
                cmd.set(row + " #BotsTag.Visible", true);
            }
